...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.mode.set('Full (extraer + transformar)')
        ...
        self.row_mode.set('por_configurable')
    ...
    def run(self):
        def stream_cmd(cmd, env):
            p = subprocess.Popen(
                ...
                bufsize=1,
                text=True,                 # reemplaza universal_newlines
                encoding='utf-8',          # <--- CLAVE
                errors='replace'           # <--- por si se cuela un byte raro
            )
        ...
        def _worker():
            try:
                ...
                # --- SQL: activar subida y parámetros ---
                env['USE_SQL'] = 'true'  # dejamos de generar Excel y subimos a SQL
                env['MSSQL_TABLE'] = 'dbo.Ventas_Solidez_Magento_2025'

                # Usa la MISMA modalidad de conexión que tus otros scripts.
                # Si ya tienes MSSQL_CONN en tu .env, la heredamos; si no, aplica el fallback:
                env['MSSQL_CONN'] = os.getenv(
                    'MSSQL_CONN',
                    'mssql+pyodbc://@localhost/TU_BASE?trusted_connection=yes&driver=ODBC Driver 17 for SQL Server'
                )

                # Borrado por el mismo campo de fecha que elegiste arriba:
                env['SQL_DELETE_BY'] = 'created' if env['DATE_FIELD'].startswith('created_') else 'updated'
                ...
                self._append_log(
                    ...
                    f"SQL={env['USE_SQL']} | TABLE={env['MSSQL_TABLE']} | DELETE_BY={env['SQL_DELETE_BY']}\n"
                )
