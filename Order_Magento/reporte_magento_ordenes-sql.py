...
# -*- coding: utf-8 -*-
"""
Magento -> SQL Server (Ventas_Solidez_Magento) v3.1
- Fetch optimizado con fields= y sesión keep-alive
- Shipping a nivel ORDEN (base_shipping_amount) con fallback a payment
- DataFrame con una fila por ítem padre (configurable) + order_item_id (PK)
- Upsert idempotente (MERGE) a SQL Server
- Logging a archivo (rotativo) + visibilidad de progreso (tqdm + per-página)
"""
...
# LOGGING
# =========================
def setup_logger(run_ts: str, log_dir: str = "."):
    log_path = os.path.join(log_dir, f"log_Ventas_Solidez_Magento_{run_ts}.log")
...
def build_session():
    ...
    retry = Retry(
        total=0, connect=3, read=0, backoff_factor=0.5,
    ...
    sess.headers.update({
        ...
        "User-Agent": "DigitalImpact-MagentoSQL/3.1"
    })
    return sess
...
def get_nested_value(data, path):
    ...
    for key in keys:
        if isinstance(cur, list):
            ...
            if m:
                key, idx = m.groups()
                cur = [item.get(key) for item in cur if key in item]
                try:
                    cur = cur[int(idx)]
...
def obtener_source_y_qty_desde_additional_info(order_json, sku):
    try:
        ...
        if not adicional_info_str:
            return None, None
        adicional_info = json.loads(adicional_info_str) if isinstance(adicional_info_str, str) else adicional_info_str
...
# =========================
# Fetch con paginación y logs
# =========================
def obtener_ordenes(fecha_inicio, fecha_fin, logger: logging.Logger):
    PAGE_SIZES_TRY = [PAGE_SIZE, 50, 25]
    PAGE_RETRIES = 5; BACKOFF_BASE = 2.0; JITTER = 0.25
    TIMEOUT = (10, 120)
    ...
    pagina = 1; total_paginas = None; pbar = None; failed_pages = []

    while True:
        page_ok = False
        for ps in PAGE_SIZES_TRY:
            for intento in range(1, PAGE_RETRIES + 1):
                auth = obtener_autenticacion(); url = build_url(pagina, ps)
                try:
                    resp = session.get(url, auth=auth, timeout=TIMEOUT)
                except requests.RequestException as e:
                    msg = f"[RETRY] pág {pagina} size={ps} excepción {e} | intento {intento}/{PAGE_RETRIES}"
                    errores.append(msg); logger.warning(msg)
                    if intento >= PAGE_RETRIES:
                        msg2 = f"❌ Página {pagina} (size={ps}): excepción definitiva tras {intento} intentos"
                        errores.append(msg2); logger.error(msg2)
                        break
                ...
                if resp.status_code == 200:
                    ...
                    if total_paginas is None:
                        ...
                        tprint(f"[INIT] total_count={total_count} | page_size={ps} | total_paginas≈{total_paginas}")

                    items = data.get("items", []) or []
                    todas.extend(items); page_ok = True
                    if pbar: pbar.update(1)

                    msg_ok = f"[PAGE] {pagina} size={ps} OK: {len(items)} órdenes (acum={len(todas)})"
                    logger.info(msg_ok); tprint(msg_ok)
                    ...
                    if resp.status_code in RETRIABLE and intento < PAGE_RETRIES:
                        ...
                        logger.warning(msg + f" | reintento {intento}/{PAGE_RETRIES}")
                        time.sleep((BACKOFF_BASE ** intento) + random.random() * JITTER)
                        continue
                    else:
                        errores.append("❌ " + msg + " | sin más reintentos")
                        logger.error("❌ " + msg + " | sin más reintentos")
                        break
            if page_ok: break
        ...
        if total_paginas is not None and pagina >= total_paginas: break
        pagina += 1

    for page in failed_pages:
        for intento in range(1, PAGE_RETRIES + 1):
            auth = obtener_autenticacion(); url = build_url(page, 25)
            try:
                resp = session.get(url, auth=auth, timeout=(10, 150))
            except requests.RequestException as e:
                msg = f"[RETRY-FINAL] pág {page} excepción {e} | intento {intento}/{PAGE_RETRIES}"
                errores.append(msg); logger.warning(msg)
                if intento >= PAGE_RETRIES:
                    msg2 = f"❌ Reintento final pág {page}: excepción definitiva"
                    errores.append(msg2); logger.error(msg2)
                    break
                time.sleep((BACKOFF_BASE ** intento) + random.random() * JITTER); continue
            if resp.status_code == 200:
                data = resp.json(); items = data.get("items", []) or []
                todas.extend(items)
                okm = f"[PAGE-FINAL] {page} size=25 OK: {len(items)} órdenes (acum={len(todas)})"
                logger.info(okm); tprint(okm)
                break
            else:
                if intento >= PAGE_RETRIES:
                    body_preview = (resp.text or "")[:300].replace("\n", " ")
                    msg = f"❌ Reintento final pág {page}: HTTP {resp.status_code} | cuerpo: {body_preview}"
                    errores.append(msg); logger.error(msg)
                else:
                    time.sleep((BACKOFF_BASE ** intento) + random.random() * JITTER)
    if pbar: pbar.close()
    return todas, errores
...
def cargar_mapeo_locales(path_file):
    ...
    req = {"id_tienda", "local"}
    if not req.issubset(set(cols_lower.keys())): return {}
    c_id = cols_lower["id_tienda"]; c_nm = cols_lower["local"]
    df = df[[c_id, c_nm]].dropna(subset=[c_id])
    df[c_id] = df[c_id].astype(str).str.strip(); df[c_nm] = df[c_nm].astype(str).str.strip()
    id_to_local = {}
    for _, row in df.iterrows():
        k = row[c_id]; v = row[c_nm]
        if not k or not v: continue
        id_to_local[k] = v
        if k.isdigit(): id_to_local[str(int(k))] = v
    return id_to_local

def generar_dataframe_ordenes(ordenes, campos_orden, campos_item, name_map, desc_map, locales_map, logger: logging.Logger):
    filas = []; errores = []
    for orden in tqdm(ordenes, desc="Procesando órdenes"):
        datos_orden = {}
        for nombre_columna, ruta in campos_orden.items():
            datos_orden[nombre_columna] = extraer_valor_con_ruta(orden, ruta)
        ...
        if not info:
            ...
            for s in sources:
                ...
                if not parent_weights_src: continue
                keys = list(parent_weights_src.keys())
                weights = [parent_weights_src[k] for k in keys]
                cuota = reparto_sources.get(s, 0)
                reparto_src = _allocate_by_weights_cents(cuota, weights, keys)
        ...
        if suma != total_shipping_cents:
            msg = f"[WARN] Shipping no cuadra: {suma} vs {total_shipping_cents} | orden {orden.get('increment_id')}"
            errores.append(msg); logger.warning(msg)
        ...
        for item in items:
            ...
            fila["order_item_id"] = item.get("item_id")

            try:
                op = item.get("base_original_price"); p = item.get("base_price")
                fila["price_discount"] = round(op - p, 2) if op and p else None
            except Exception:
                fila["price_discount"] = None

            shipping_split = (shipping_cents_por_parent.get(item.get("item_id"), 0) or 0) / 100.0
            for nombre_columna, ruta in campos_item.items():
                if nombre_columna == "price_discount":
                    ...
                    fila[nombre_columna] = buscar_valor_item(item, ruta, orden)

            sid_raw = fila.get("source_id", "")
            sid_key = normalizar_source_id(sid_raw)
            fila["source_Name"] = locales_map.get(sid_key, "") or locales_map.get(str(sid_raw).strip(), "")

            try:
                price = float(fila.get("price") or 0)
                qty_ordered = float(fila.get("qty_ordered") or 0)
                coupon_discount = float(fila.get("coupon_discount") or 0)
                grand_total_item = (price * qty_ordered) + shipping_split - coupon_discount
    ...
    for col in ["created","invoice_date","last_change_date"]:
        if col in df.columns:
            df[col] = pd.to_datetime(df[col], errors="coerce")
    for col in ["original_price","price","row_total","total_shipping_charges","coupon_discount",
                "price_discount","qty_ordered","qty_confirmed","payment_value",
                "grand_total_purchased","grand_total_item"]:
        if col in df.columns:
...
# SQL MERGE
# =========================
def upsert_dataframe_mssql(df, engine, schema, table, logger: logging.Logger):
    df = df.copy()
    df["_ingested_at"] = pd.Timestamp.utcnow()

    stg = f"{table}_stg_{int(time.time())}"
    logger.info(f"[SQL] Cargando staging {schema}.{stg} ({len(df)} filas)")
    df.to_sql(stg, engine, schema=schema, if_exists="replace", index=False, method="multi", chunksize=5000)

    cols = list(df.columns)
    col_list = ", ".join([f'[{c}]' for c in cols])
    set_updates = ", ".join([f't.[{c}] = s.[{c}]' for c in cols if c != "order_item_id"])
    insert_vals = ", ".join([f's.[{c}]' for c in cols])

    sql = f"""
    MERGE [{schema}].[{table}] AS t
    USING [{schema}].[{stg}] AS s
    ...
    DROP TABLE [{schema}].[{stg}];
    """
    with engine.begin() as conn:
        conn.execute(text(sql))
    logger.info(f"[SQL] MERGE completado en {schema}.{table}")
...
if __name__ == "__main__":
    ...
    tprint(f"[LOG] archivo: {log_path}")

    name_map, desc_map = cargar_mapeo_reglas(SALES_RULES_PATH)
    locales_map = cargar_mapeo_locales(LOCALES_PATH)
    tprint(f"[ACCION] Locales cargados: {len(locales_map)}")
    
    fecha_inicio, fecha_fin = calcular_rango_fechas(dias_atras)
    tprint(f"[INIT] Rango: {fecha_inicio} -> {fecha_fin}")

    ordenes, errores_api = obtener_ordenes(fecha_inicio, fecha_fin, logger)
    if errores_api:
        logger.warning(f"[WARN] Errores API: {len(errores_api)}")
        for e in errores_api[-10:]:
            logger.warning(e)
        tprint(f"[WARN] Errores API: {len(errores_api)} (ver log)")

    df, errores_tx = generar_dataframe_ordenes(ordenes, campos_orden, campos_item, name_map, desc_map, locales_map, logger)
    tprint(f"[INFO] Filas para upsert: {len(df)}")
    if errores_tx:
        logger.warning(f"[WARN] Errores transformación: {len(errores_tx)}")
        for e in errores_tx[-10:]:
            logger.warning(e)

    upsert_dataframe_mssql(df, engine, schema, table_name, logger)
    tprint(f"[ACCION] Upsert completado en {schema}.{table_name}")
    tprint(f"[FIN] OK | log: {log_path}")
