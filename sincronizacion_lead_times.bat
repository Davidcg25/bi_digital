...
pushd "%ETL_DIR%"
"%PY%" "%ETL_DIR%\etl_lead_times.py" >> "%LOGFILE%" 2>&1
set "RC=%ERRORLEVEL%"
popd

for /f "usebackq delims=" %%I in (`powershell -NoLogo -NoProfile -Command "(Get-Date).ToString('yyyy-MM-dd HH:mm:ss')"`) do set "TS=%%I"
if %RC% neq 0 (
  echo [%TS%] ERROR - etl_lead_times termino con RC=%RC% >> "%LOGFILE%"
  exit /b %RC%
)

echo [%TS%] OK - Lead times finalizado correctamente >> "%LOGFILE%"
exit /b 0
